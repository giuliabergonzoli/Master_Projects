...
ggplot(data=studentsData, aes(x=as.factor(country), y=read, fill=as.factor(country))) +
  geom_boxplot() +
  labs(x='Country', y='Reading Achievement') +
  ggtitle('Boxplot of reading achievements among countries') +
  theme_minimal() +
...
lm1 <- lm(read ~ gender + immigration + language + hisced + grade_rep + fear_failure + belonging + bullied + 
            + ESCS_status + teacher_support + emo_sup + learn_time_read + 
            + immigration:grade_rep + immigration:fear_failure + immigration:belonging +  
            + immigration:teacher_support + immigration:emo_sup, data = studentsData)
summary(lm1)
...
lmm1 = lmer(read ~ gender + immigration + language + hisced + grade_rep + fear_failure + belonging + bullied + 
              + ESCS_status + teacher_support + emo_sup + learn_time_read + 
              + immigration:grade_rep + immigration:fear_failure + immigration:belonging +  
              + immigration:teacher_support + immigration:emo_sup + (1|country), 
            data = studentsData)
summary(lmm1)
